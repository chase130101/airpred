# read in data
sensor_census = read.csv('../data/subset_sensor_census_toImpute.csv')
...
# imputing with mean
require('mice')
sensor_census_imputed_mean = complete(mice(sensor_census, method = 'mean', maxit = 1, m = 1))

# add site and MonitorData back in
sensor_census_imp = cbind(site_MonitorData, sensor_census_imputed_mean)

# write to csv for modeling in python
write.csv(sensor_census_imp, '../data/sensor_census_imputed_mean.csv', row.names = F)
