...
#impute_variables = select(full_data, -c(date, month, MonitorData))
#other_variables = select(full_data, c(date, month, MonitorData))
...
#Impute missing values with mean for that site
imputed_data_groupmean = full_data %>% 
  group_by(site) %>% 
  mutate_at(.funs = funs(impute_mean(.)), .vars = vars(-(year:MonitorData))) %>%
  ungroup()

imputed_data_groupmean = as.data.frame(imputed_data_groupmean)

#Impute any remaining missing values with overall mean
for(i in 9:ncol(imputed_data_groupmean)){
  imputed_data_groupmean[,i] = impute_mean(imputed_data_groupmean[,i])
}
...
fwrite(imputed_data_groupmean, '../data/imputed_data_groupmean.csv')
